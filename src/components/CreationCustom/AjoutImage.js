...
export default function AjoutImage(props) {
  ...
  function modifierAttibut(attribut, ref) {
    if(attribut==="img") {
      props.modifierAttibut(props.index, attribut, ref.current.files[0])
    } else {
  ...
  return (
    <div className="w-full bg-gray-100 rounded p-1 flex flex-col gap-2 lg:gap-0 lg:flex-row justify-between">
      <Input onChange={() => {modifierAttibut("img", imgRef)}} reference={imgRef} label="Image" type="file" obligatoire={true} class="border-none" accept=".png, .jpg, .jpeg, .webp"/>
